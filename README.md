...
v0.2.5 targets CLIProxyAPI v7.2.152, schema 5, at host commit `c76dfd4e0edabab9000628b1560ab8ab379eadb8`. It uses native ABI v1. The registered logo is `https://raw.githubusercontent.com/DoingDog/cpa-plugin-censorship/main/logo.png`.
...
- Interactions explicit text paths: documented `system_instruction` or fallback camel-case `systemInstruction`, recursive documented input text subsets, and direct input object or array items with exact `type: "text"` and text content. Annotated `model_output.content` text remains blockable, but a final length-changing `strip` or `obfs` on a non-empty `annotations` array returns local `censorship_invalid_request` with `censorship cannot rewrite annotated text`; empty annotations add no restriction.
