...
- Protects annotated Interactions `model_output.content` text: `block` remains allowed, while a final length-changing `strip` or `obfs` on non-empty annotations returns local `censorship_invalid_request`; empty annotations remain rewritable.
